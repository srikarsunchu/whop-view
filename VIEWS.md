...
`wv gtm launch <prod_id> [--budget 40] [--creative file_x …]` (`src/views/launch.ts`) is the launch-day playbook as one plan: four writes in order, one approval, one rerun. The skill used to teach it as eight shell lines an agent had to sequence; now the sequence is code with a fixture and a test.
