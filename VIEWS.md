...
`wv` wraps the Whop CLI for both of its callers, and seven skills put an agent to work on top of it. For an agent or a script on a pipe, every read is forwarded as `whop`'s own bytes and the CLI's gaps are filled around them: a write comes back as a plan to approve with a signed `rerun` ([Agent gate](#agent-gate)), the exit code says what went wrong, every screen answers as JSON, `--all` follows the cursor, `wv agent <group>` is a manifest that fits a context window ([Agent manifest](#agent-manifest)), and `wv --mcp` serves all of it as four tools on stdio ([MCP server](#mcp-server)). For a person at the terminal, it asks `whop` for JSON and renders a table, a card, a confirmation, or an error. Each skill stands on one screen that joins the reads its loop needs and on recipes that turn a playbook into one plan with one approval: [GTM](#gtm-view) with launch, winback, and rank ([Recipes](#recipes)); [Store](#store) with price and publish; [Money](#money) with the month-end close; [Support](#support) with lookup, refund, and dispute; [Dev](#dev) with the webhook proof; and [Doctor](#doctor-view) as every skill's preflight. One core serves both faces: the inference rules and hint files that classify a response for the terminal are what the plans and the JSON screens are built from, and the [passthrough rules](#passthrough-rules) decide which face a call gets. The skills live in `skills/`, one `SKILL.md` of rules and a map each, with playbooks, a failure map, and a command reference generated from the manifest under `references/`, and evals under `evals/` that run the scenarios through `claude -p` against the stand-in `whop` in `tests/fake-whop.sh`. Written against `whop` 0.18.2, API 2026-09-15, from real envelopes captured on 2026-09-19 and 2026-09-21.
...
`wv money` (`src/views/money.ts`) is the treasury on one screen, the "know" stage of the whop-money skill. `gatherMoney` in `bin.ts` runs `auth status`, `payouts methods --include_limits`, `payouts list --first 5`, `accounts reserves`, and `verifications list`, and `ledgers list --first 100`, then one `ledgers report --report_type balance_summary --currency <c>` per currency the account holds money in: `currenciesOf` joins `usd`, the currency of every saved method, every currency on the ledger's last page (`currency.code` on a line), and every reserve. The ledger read is what makes a EUR sale visible on an account whose only method pays USD; without it the screen showed one balance and hid the rest. `payableCurrencies` is the set a saved method delivers, and `unpayable` is every balance with money in a currency outside that set: the row is marked `no payout method in this currency` in `warn`, `moneyData` carries `payable` per balance and `unpayable` as a list, and the footer names the swap for each one. With no method at all nothing is marked, since the methods row already says so and a swap would not help. `balanceOf` reads `available` and keeps the other rows the report names; `limitsOf` reads both speeds' limits off the page's `limits` sibling, with the block behind a zero (`kyc_completed`, `restricted_account`); `pickMethod` is the rule the close and the payout gate share: the named method, else the default, else the only one, and a refusal in words otherwise. The screen: balances per currency, the limits per speed with `blocked · <Whop's words>` in `bad`, the saved methods with the default marked, reserves when any, the last five payouts as a table, a note when payouts are blocked, and a footer with the identity fix, `wv money close --plan`, and every read. `--format json` or a pipe is `moneyData`: the same plus `payoutsBlocked` named as data.

`wv money close [--keep N] [--method potk_x] [--currency usd] [--speed standard|instant] [--period last|this] [--notes …]` is the third recipe: an `exports create --resource financial-activity` filtered to the currency and the window (`closeWindow`: the previous calendar month in UTC, or the month so far), then a `payouts create` of what is available above `--keep`, rounded to cents, to the picked method at the chosen speed. Blockers before any write: the balance unreadable, nothing above the floor, no method or an ambiguous one, Whop's block on payouts (skipped in sandbox), the amount over Whop's limit for the speed, the amount over `WV_PAYOUT_CAP`. The person types the payout amount back. `plan.data` carries the balance, the floor, the amount, the method, the limit, the cap, and the window. Done-when: `exports get` `completed` with a `download_url`, `payouts get` `completed`, `wv money` at the floor. `wv money swap --from eur --to usd --amount 80` is the fourth recipe, the way out for a stuck balance: `swaps quote --amount --from_token --to_token` at plan time (compute-only, so it is a read the gate does not hold), then one `swaps create` step at the quoted rate. Fiat pairs fill at Whop's mid-market rate, the same the quote returned. `quoteOf` reads `amount_in`, `amount_out`, `rate`, `fee_bps`, `fee_amount` (strings, no `id`, so the classifier files the payload under `other`). The card: both balances before and after (`from  €80.00 available → €0.00`, `to  $1,234.56 available → $1,326.56`), the rate with the fee or `no fee`, the swap line. Blockers: the from-balance unreadable, more than it holds, no quote. Warnings: a fee (the after is net of it), the to-balance unreadable. The person types the amount back in the from currency. `plan.data` carries both balances, the quote, and `after`. Done-when: `swaps get` at `completed`, `wv money` moved. Scenes `money`, `money.ready` (EUR 80 with a USD-only method: marked, and the footer's swap), `close.blocked`, `close.ready`, `swap.ready`, `swap.blocked`; the fixture `payouts.methods.ready` is synthetic (one default method, a $5,000 standard limit), since this account has no method and a blocked limit, and the quote in the scenes is synthetic at 1.15 (the recorded live quote on 2026-09-21 was usd→eur at 0.87, no fee).

The whop-money skill (`skills/whop-money`) follows the gtm layout: a SKILL.md of money rules and a map, references for the payout, the close, the swap, reconciliation from the ledger, the failure map, the generated command map, and `gate.md`. `skills/_shared/gate.md` is the rules every wv skill shares (the envelope, the approve token, idempotency, exit codes, preflight, production by default); `pnpm skill` copies it into each skill's `references/` and regenerates each skill's command map from its own group list, then installs both skills. `pnpm eval whop-money` runs the money scenarios: a payout refused by the identity limit, a close blocked by it, a close approved on a ready account (export then payout, in order), and a read-only balance question.
...
`wv store --from DE` is the buyer's side: `storeFrom` in `bin.ts` reads the country, `taxablePlans` picks every plan with a price (at most eight, so a large catalog is not a storm of calls), and one `plans calculate_tax <id> --address {"country":"DE"}` runs per plan, in parallel. The preview is `{ currency, tax_behavior, subtotal, tax_amount, total, status }` in minor units with no `id`; `taxOf` reads it into whole units and `localizedLine` puts `from DE: $11.90 · $1.90 tax (19%)` or `from BR: $10.00 · no tax` at the end of the plan line, or the error in Whop's words. `storeData` carries `from` and `localized` per plan. The footer offers `wv store --from <CC>` when nothing was asked. Recorded on 2026-09-21: the $10 plan is $11.90 from Germany and $10.00 from Brazil. Scene `store.from`.
...
`wv report` (`src/views/report.ts`) is the Monday brief as one read, composed from the other screens' gather functions. `gatherReport` in `bin.ts` runs six metrics (`page_visits`, `new_users`, `gross_revenue`, `net_revenue`, `ad_spend`, `churn_rate`) over the last seven whole days and the seven before, plus `economic-intelligence list --status ready`, then `gatherDoctor`, `gatherGtm`, `gatherMoney`, and `gatherStore` in turn (each has its own spinner), then `rankReads` for up to three `active` campaigns. About forty reads, no writes. `total` reduces a series to one number (counts and money sum, rates average the points that exist; no points is zero, an error is an error), `kpis` pairs the windows with a relative change (none against a zero week), and `nextActions` derives the brief's list from everything read: failing doctor fixes, launch gap fixes, the one action `rankSummary` picks per campaign (a pause first, then a scale, then a rejected or silent group), an approve command per ready recommendation, the identity fix when payouts are blocked.

Three faces: the page (`reportView`: the KPI table with change colored by whether up is good, a status section for money, store, and people, a blockers section from the doctor and the gaps, one line per campaign with its verdict counts, the recommendations, and the next actions as the footer), the data (`reportData`, `--format json` or any pipe), and Markdown (`reportMarkdown`, `--md`, a wv flag stripped in `ownFlags`, the same facts with the commands in backticks, for a schedule that posts it). `report` is in `OURS`; it is not in `DATA_SCREENS` because its Markdown face needs the flag, so `main` and `agentMain` route it to `reportRun` before the screen check. Exit 0 when every metric read succeeded, 1 otherwise. Scene `report`; the Markdown is snapshotted at `tests/snapshots/report.md`.

The whop-report skill (`skills/whop-report`) is the shortest: six rules (one call three faces, compare do not narrate, next is the brief, recommendations are Whop's and the person decides, a brief can run unattended, say when a read failed), the loop, two playbooks (the brief, the schedule), the failure map, the generated command map, and `gate.md`. `pnpm eval whop-report` runs a read-only brief, a brief with an approved recommendation (the one write, gated), and a Markdown brief that must not act on its own `next` list.

## Setup

`wv setup` (`src/views/setup.ts`) is the first hour as a numbered list, built on the doctor's checks. `stepFor` says who does each check that is not green: `cli` (a wv write the person approves: intelligence, a product, a profile switch, a webhook), `terminal` (an interactive `whop` command that owns the terminal: `login`), `browser` (only the person, at a URL: the ads payment method), or `both` (wv starts it and the person finishes in a browser: identity verification, the payout method, the pixel, the Facebook page, a new API key). Blocking checks come first. Each step carries the doctor's detail as `why`, one sentence of what to do as `then`, the command to run, and the URL to open. `setupData` is the JSON face with `green`, `steps`, and `blocking`; exit 1 while steps remain. A green account prints one line and points at `wv report`. Scenes `setup`, `setup.green`.

The whop-setup skill (`skills/whop-setup`) is the onboarding: seven rules (the list is the plan, blocking means no sale, the person does the browser steps, wv runs the CLI steps as plans, interactive commands run as `whop`, re-check do not assume, the first sale needs the first three plus a link), a table of the ten steps with who does what, the walkthrough with what each step looks like when it lands, the failure map, the generated command map, and `gate.md`. `pnpm eval whop-setup` runs a read-only "what do I do first" and a "do what you can" that must run only the one CLI-doable write (intelligence) and hand the browser steps back.

## Doctor view

`wv doctor` (`src/views/doctor.ts`) is ten checks on one screen, exit 1 when a blocking one fails. `bin.ts` runs `auth status` first because `permissions check` needs the account id, then nine reads in parallel plus `permissions check --resource_id <biz> --actions <DOCTOR_ACTIONS>`, then `webhooks deliveries <id> --first 20` for up to three webhooks. `checks(input)` is pure and returns `{ key, label, level, detail, fix?, dashboard?, blocking }` per check; `doctorView` paints them.

| check | read from | ok | fail or warn | fix |
|---|---|---|---|---|
| signed in (blocking) | `auth status` | `loggedIn` | anything else | `whop login` |
| payout method | `payouts methods`, `payouts supported-methods --first 100` | a saved method, named | none saved: what Whop would let this account add (name and country of the first three), or that it lists none, which is the non-US seller's first wall: the account's country and identity decide what is offered | `whop payouts supported-methods`, then `payouts create-method` |
...
Rule 1 has two exceptions. The agent gate below: a write in a pipe is not passed through until it carries `--yes`. And `--format human`, wv's sixth format beside whop's `toon`, `json`, `yaml`, `md`, and `jsonl`: `ownFlags` strips it and `main` treats the call as if a terminal were there, so a pipe gets the rendering at `--width` or 80 columns and, by `colorEnabled`, no color unless `FORCE_COLOR`. It does not touch rule 2: the flag is gone before the rules run, and whop is asked for `--format json --full-output` as always. An empty argv keeps the pipe's answer, since the session needs a terminal. This is the flag the CLI would grow if the rendering moved inside it.
...
`wv --mcp` (`src/mcp.ts`) is the agent face on the transport every desktop client speaks. `whop --mcp` already serves 298 tools, one per command, and `payouts_create` on it sends money on the first call: the 149 commands `--llms-full` tags "confirm with the user" are enforced nowhere. The person Whop's CLI serves is a developer at a terminal; the person who pays Whop talks to Claude desktop or Cursor and never opens one, and MCP is the only way the gate reaches them. So the server is not a port of the pipe. It is the pipe's contract, delivered where its customer is.

**One core.** `agentMain` used to print and exit. Now `agentReply(argv, { mode, env, plan, all, md })` in `bin.ts` is pure: it returns `{ kind: "text", text, code }` (exactly the bytes the pipe prints: an envelope, a screen's data, the manifest) or `{ kind: "exec", argv, env }` (the `whop` argv to run, wv's flags stripped). `agentMain` is now four lines over it: print and exit, or `passthroughPiped`. The screens, `gtm rank`, `support lookup`, `report`, `--all`, and the recipes each return a `TextReply` too (`screenReply`, `rankReply`, `lookupReply`, `reportReply`, `allPagesReply`, `recipeReply`), and the printing wrappers the terminal uses are one line each. The pipe tests are the proof nothing moved: byte-identical before and after.

**Four tools, every one a wv argv underneath** (`argvFor` is the whole mapping, and it is tested):

- `wv_manifest { group?, format? }` is `wv agent [group]`.
- `wv_read { argv, all?, sandbox? }` runs a read with `--format json --full-output` added unless the caller chose a format, so the answer has `ok` and `error` like every other reply. A write here is refused with a pointer to `wv_write`, and the check runs on the argv with `--yes` and `--approve` stripped first, so consent cannot be smuggled through the read tool.
- `wv_screen { screen, args?, markdown?, sandbox? }` is `doctor`, `gtm`, `money`, `dev`, `store`, `setup`, `report` (`--md` with `markdown`), `gtm rank`, and `support lookup`.
- `wv_write { argv, plan?, sandbox? }` is a write or a recipe. The first call is the plan and the `rerun`; the second call is the `rerun` array as given (a leading `wv` is dropped), and the token binds it to that exact argv, mode, and machine within the TTL, exactly as in the pipe. `plan: true` is `--plan`.

**Results.** `toolResult(text, code)` puts the bytes in `content` as text and, when they parse as one JSON object, in `structuredContent` too. `isError` is set when the reply carries `error` and no `rerun` (a refusal, a whop error, a bad approval) or, for raw bytes, when the exit code is not zero. A plan waiting for consent is not an error, and neither is a screen whose `ok` is false: a business that is not set up is data. An exec is `runBuffered` in `runner.ts`, the pipe's spawn with stdout collected and the same `agentExitCode` over the tail.

**Protocol.** JSON-RPC 2.0, one message per line on stdio, hand-rolled because it is four methods: `initialize` (answers the client's version when it is one of the three known, the newest otherwise, with `instructions` that state the gate in five sentences), `ping`, `tools/list`, `tools/call`; notifications are dropped; anything else is `-32601`. Requests run concurrently and answer as they finish; when stdin closes the calls still running finish first, then the process exits (the first probe lost two answers to an eager exit, which is why `pending` exists). No dependency was added.

**Tests.** `tests/mcp.test.ts` spawns `bin.ts --mcp` against the fake `whop` and proves: the handshake and the tool list; the argv mapping; a write is a plan first and never reaches whop, the rerun runs it with neither `--approve` nor the token in whop's argv, and an edited rerun is `APPROVAL_INVALID`; `plan` mints no rerun and a refusal has none; the read tool refuses a write carrying `--yes` or `--approve`; a screen carries wv's `meta`; a recipe plans through the write tool; and `handle` answers a bad method in protocol. For elicitation the harness plays the client: it declares the capability, answers each `elicitation/create` with what the person would have done, and closes stdin only after every request has its answer. Proven: a yes runs the write in the same call and the model never sees a rerun; no, cancel, and a false answer run nothing and come back `DECLINED` with no rerun; a payout asks for the amount, `50` for a $5 payout sends nothing and `$5.00` sends it; a client without the capability gets the plan and the rerun; `plan` never asks; the sandbox asks for a yes, not an amount, and the approved rerun runs there. `mcp add` is checked against the fake whop's argv echo, and `mcp doctor` is run for real through the pipe face.

**Elicitation: the approval leaves the model.** In the pipe, consent is a token the model copies into a rerun, and the person's yes happens in chat, where the model relays it. MCP has `elicitation/create` (spec 2025-06-18): the server sends the client a message and a small schema, the client puts it in front of the person in its own UI, and the answer comes back to the server. A client declares it at `initialize` (`capabilities.elicitation`), and `handle` remembers that for the session. When it is there, `wv_write` does what the terminal does: the reply that would have been `CONFIRMATION_REQUIRED` becomes a question instead. `elicitParamsFor(plan, mode)` builds it from the plan the pipe already produces: `planMessage` is the card as lines (the command, production or sandbox with the account, the money and its destination, an ad plan's commitment, the changes against the record, a recipe's steps with the skipped ones left out, the warnings), and the schema is the terminal's rule: money in production asks for the amount typed back (`amountMatcher`, so `$5.00`, `5`, and `5.00` all match and `50` does not), and everything else asks for a yes. `consented` is that rule as a function. A yes runs the rerun inside the same tool call, in the same mode it was planned in (the rerun does not carry `--sandbox`, so `withConsent` passes the mode along or the sandbox token would be checked against production and refused), and the tool's answer is whop's answer to the write; the model never sees a token. A no, a dismissed prompt, or a wrong amount comes back as `DECLINED` with the plan and no `rerun`, so the model can report what was refused and cannot run it anyway. `--plan` never asks. A client that did not declare elicitation gets the pipe's reply: the plan and the rerun, and the model shows the plan and calls again. The server's side of the protocol is in `serveMcp`: outbound requests get their own ids, a line with no `method` is a response and resolves the waiting call, an error or a shape the spec does not allow counts as cancel, and closing stdin cancels every question still open so the calls finish before the exit. Logged to stderr when the person approves, since stderr is the server's log.

**`wv mcp add` and `wv mcp doctor`.** `whop mcp add` takes `--command` for exactly this: `mcpAddArgv` prepends `--command "wv --mcp"` unless the person gave one, and execs whop's installer with the rest (`--agent claude-code`, `--no-global`) as given, so wv registers through the path Whop documented. One thing the installer decides that wv cannot: the entry is named `whop`, so it replaces Whop's server rather than sitting beside it. To have both, `claude mcp add -s user wv -- wv --mcp` names it `wv`; the README leads with that. Found on the first real registration, 2026-09-21, together with the fact that `tsc` writes `dist/bin.js` without the execute bit the `wv` link needs, which the build script now sets. `mcpDoctor` spawns this same program with `--mcp`, sends `initialize` and `tools/list`, and reports `ok`, `toolCount`, the tools, the server's name, version, and protocol version, and the register command, the way `whop mcp doctor` reports for whop's server; JSON in a pipe, lines in a terminal, exit 1 if the server does not answer within fifteen seconds or lists nothing. It proves the startup path a client takes, not just that the module loads.

**A model's `--yes` is not consent.** The pipe honors `--yes` for a script a person wrote on purpose. Through the server the caller is a model, so `argvFor` drops `--yes` from `wv_write`'s argv before the gate sees it, and the write is planned like any other. `WV_MCP_NO_ELICIT=1` makes the server ignore a client's elicitation capability: a headless client, `claude -p` in the evals, declares it and has nobody to ask, and without the override every write would come back `DECLINED`.

**Evals over MCP.** `pnpm eval <skill> --mcp` (`scripts/eval-skill.ts`) runs the same scenarios with no shell: `--mcp-config` names this repo's `bin.ts --mcp` with the fake `whop`, the log, the caps, the secret, and `WV_MCP_NO_ELICIT` in its env; `--strict-mcp-config` and `--disallowedTools Bash` leave the agent the four tools and nothing else; the workspace `CLAUDE.md` says which tool stands for which `wv` command. Each `mcp__wv__*` tool call is rendered back into the `wv` argv it stands for (`asCommand`), so the judge reads a tool call the way it reads a Bash line, and the report goes to `evals/results.mcp.md` beside `results.md`. The first run, whop-dev, 2026-09-21: 15/15 on both transports. The OAuth webhook scenario took seven tool calls and no reference-file reads over MCP, against nine shell commands and two `cat`s over Bash; the plan came before the rerun on both, `--yes` never appeared, and the only writes that reached the fake were the profile switch and the webhook create with the planned idempotency key. The typed schemas did what the command map was written to do.
...
| # | rule | detect | render |
|---|---|---|---|
| 1 | hidden | key in `HIDDEN` or in hints `hidden`, or the spec's kind is `hidden` | never shown |
| 5b | the spec's word | `hints.kinds[key]` from `_spec.json`: `count` renders as a number, `money` as money (a `formatted` hint still wins), `status` as a badge, `date` as a date, `plan` through `planPrice` | ahead of rules 2 to 4 and 6 to 9, after rule 5 so the ledger's `usd_amount` still wins |
...
Two layers. Under every hand file sit the kinds the OpenAPI spec settles, derived once by `pnpm spec` (`scripts/spec.ts`) from `https://api.whop.com/openapi.json` into `src/hints/_spec.json`: per CLI command, the response schema its rows come from (`data[]` items on a page, `data` on a wrapped record, else the body) and, per field, `id` (the key `id`, a description saying "prefixed `prod_`", or an id-shaped example), `money` (the `Money` schema, or a number or string whose description says amount, major units, cents, USD), `date` (`format: date-time`, or a description saying ISO 8601 or timestamp), `status` (a string enum under a status-like key), `relation` (an object with an id and a title, name, or username), `plan` (an object with `plan_type` and a price), `count` (a number whose description says count, days, seconds, basis points, so it is never mistaken for money), and `hidden` (deprecated, internal, a currency code beside its amount, an array of images). Operations map to commands by path (`/ad_campaigns/{id}/pause` is `ad-campaigns pause`, `/products` GET is `products list`) with a short alias table for the groups the CLI names differently (`/financial_activity` is `ledgers list`, `/stats/{metric}` is `stats get`, the hyphenated `payouts create-method` family). On 2026-09-21 that is 370 commands across 84 groups from 296 paths; 46 CLI commands have no derived kinds, and they are the CLI-only verbs (`auth`, `apps dev`, `skills`, `webhooks test`) plus a handful the spec answers with no record. `infer` consults the derived kind first (rule 5b below, after the ledger's own amount rule), so the regex guesses are the fallback, not the rule. `WV_NO_SPEC` turns the layer off; `WV_NO_HINTS` drops the hand files; `hintsWith` takes both switches for the coverage run.

The hand file, `src/hints/<resource>.json`, or `src/hints/<resource>.<verb>.json` when one verb returns rows that share nothing with the group's list (`payouts methods`, `cards transactions`, `accounts reserves`, `partners list`), says only what the spec cannot: which field leads (`primary`), which columns and in what order (`columns`), the label a person would use (`labels`: `member_count` is `members`, `current_period_end` is `renews`), the relation to show (`relation`), a preformatted string to prefer (`formatted`), and the fields to hide that the spec does not mark (a campaign's thirty metric columns). `pnpm spec --trim` removes from every hand file the facts the derived kinds already settle and deletes a file left empty; on 2026-09-21 that took the 49 files from 1,050 facts to 920 with the snapshots byte-identical, and deleted none, because every file also names its columns and labels, which are product choices. A verb file wins whole over the group file; it never merges. Every key optional.

`docs/coverage.md` (`pnpm coverage`) is the measurement: every recorded list and record rendered three ways, with the hand file over the derived kinds (what ships), with the derived kinds alone, and with inference alone, and the columns or fields that differ; then every hand file against the derived kinds, fact by fact. The honest reading on 2026-09-21: the spec settles nearly every money, status, date, and plan fact the hand files state, so those are gone from the files; what the files still carry is column choice and labels, and one recorded list in twelve renders the same columns from the spec alone. Two generic column rules came out of the run and cost no hand file anything: a relation that is the same on every row is dropped (an account-scoped list carries the account on each row), and a `<thing>_name` string is taken as the relation spelled out. `docs/in-cli.md` is what the same code looks like inside the whop binary.
...
- Evals against the fake `whop` kept teaching the same lesson from different angles: an agent that cannot see the effect of a write will redo it by hand. The fake was stateless, so a submitted dispute read back as `needs_response` and a changed price read back as `$10.00`, and twice the agent then typed the write as raw `whop`. The fake now remembers a profile switch, a dispute submit, and a price change beside its log; the gate reference says a stale read-back is never a reason to redo a write; and every scenario checks that no write was typed as raw `whop`. The same agent, asked to respond to a dispute with no evidence in hand, tried to author a document and upload it; the support skill now says evidence is the person's documents and no evidence is a stop.
