...
**Tests.** `tests/mcp.test.ts` spawns `bin.ts --mcp` against the fake `whop` and proves: the handshake and the tool list; the argv mapping; a write is a plan first and never reaches whop, the rerun runs it with neither `--approve` nor the token in whop's argv, and an edited rerun is `APPROVAL_INVALID`; `plan` mints no rerun and a refusal has none; the read tool refuses a write carrying `--yes` or `--approve`; a screen carries wv's `meta`; a recipe plans through the write tool; and `handle` answers a bad method in protocol.

**Not yet.** Elicitation, so the server can ask the person for the yes itself and the model never holds the approval. `wv mcp add` and `wv mcp doctor` over `whop mcp add --command "wv --mcp"`. One tool per recipe with a typed input schema. The skills as MCP prompts and resources. The eval runner in MCP mode. These are the next steps, in that order.
