...
**`wv mcp add` and `wv mcp doctor`.** `whop mcp add` takes `--command` for exactly this: `mcpAddArgv` prepends `--command "wv --mcp"` unless the person gave one, and execs whop's installer with the rest (`--agent claude-code`, `--no-global`) as given, so wv registers through the path Whop documented. `mcpDoctor` spawns this same program with `--mcp`, sends `initialize` and `tools/list`, and reports `ok`, `toolCount`, the tools, the server's name, version, and protocol version, and the register command, the way `whop mcp doctor` reports for whop's server; JSON in a pipe, lines in a terminal, exit 1 if the server does not answer within fifteen seconds or lists nothing. It proves the startup path a client takes, not just that the module loads.
