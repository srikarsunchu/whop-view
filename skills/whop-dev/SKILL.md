...
1. **Webhooks need an API key, not the login.** The OAuth token `whop login` leaves behind lacks `developer:manage_webhook`, so `webhooks list` answers 403 and `webhooks create` would too. `wv dev --format json` says so in `webhookAccess` with the fix: `wv auth switch <saved api-key profile>`, or `whop auth login --method api-key`. `auth switch` changes which credential every later command uses, so wv gates it like a write: run `wv auth switch <profile>`, approve, then the webhook work, then switch back if the person wants the OAuth profile active.
