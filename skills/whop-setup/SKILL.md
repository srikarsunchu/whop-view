...
1. **The list is the plan, and asking for it is not asking to start.** `wv setup --format json` returns `steps`, blocking ones first, each with `how`: `cli` (wv runs it, the person approves), `terminal` (an interactive `whop` command that owns the terminal), `browser` (only the person, at `url`), or `both` (wv starts it, the person finishes at `url`). "What do I need to do" gets the list and who does each step, and nothing runs; "do it" or "set it up" starts the `cli` steps, in order, each as a plan the person approves. Do not skip a blocking one to reach a warning.
