...
| 2 | `SWAP_BLOCKED` | the swap cannot run as planned: more than the balance holds, no quote for the pair, the from-balance unreadable | report `plan.blockers`; `wv money` for the balances; never run `swaps create` by hand |
| 4 | `HTTP_401` · "Stored credential not found. Run `whop login` again." on `payouts list` | recorded 2026-09-21: `payouts list` refuses the OAuth login that `payouts methods`, `ledgers report`, and every other money read accept on the same screen; `wv money` shows the line in place and the rest of the screen stands | an API-key profile for the payout history; report it as Whop's inconsistency, not the person's login |
| 0 | `payouts supported-methods` returns an empty page, with or without `--country` | recorded 2026-09-21 on an unverified account: Whop lists no destination this account can add under any country; the account's country and identity decide what is offered | finish identity verification, then read again; `wv doctor` carries it as the payout method check |
