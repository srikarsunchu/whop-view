...
case "$1 $2" in
  ...
  "verifications create") echo '{"ok":true,"data":{"id":"ver_1","status":"pending","last_error_code":null},"meta":{"command":"verifications create","duration":"1ms"}}'; exit 0 ;;
  "accounts update-preferences") echo '{"ok":true,"data":{"economic_intelligence":true},"meta":{"command":"accounts update-preferences","duration":"1ms"}}'; exit 0 ;;
  ...
  "refunds list") echo '{"ok":true,"data":{"data":[{"id":"ref_1","status":"succeeded","amount":{"currency":"usd","amount":"10.00"},"payment_id":"pay_JFHAhioMdPL1ts","created_at":"2026-09-21T12:30:00Z"}],"page_info":{"start_cursor":null,"end_cursor":null,"has_next_page":false,"has_previous_page":false}},"meta":{"command":"refunds list","duration":"1ms"}}'; exit 0 ;;
  ...
  "ledgers list") fx ledgers.list.json ;;
  "payouts supported-methods") echo '{"ok":true,"data":{"data":[],"page_info":{"start_cursor":null,"end_cursor":null,"has_next_page":false,"has_previous_page":false}},"meta":{"command":"payouts supported-methods","duration":"1ms"}}'; exit 0 ;;
  "swaps quote") amt=1; i=0; for a in "$@"; do i=$((i+1)); [ "$a" = "--amount" ] && eval "amt=\${$((i+1))}"; done; out=$(awk "BEGIN{printf \"%.2f\", $amt*0.87}"); echo '{"ok":true,"data":{"object":"swap_quote","amount_in":"'"$amt"'","amount_out":"'"$out"'","amount_out_min":"'"$out"'","rate":"0.87","fee_bps":0,"fee_amount":null,"from_token":{"symbol":"usd","decimals":2},"to_token":{"symbol":"eur","decimals":2}},"meta":{"command":"swaps quote","duration":"1ms"}}'; exit 0 ;;
  "swaps create") echo '{"ok":true,"data":{"id":"swap_1","status":"completed","amount_in":"5","amount_out":"4.35"},"meta":{"command":"swaps create","duration":"1ms"}}'; exit 0 ;;
  "swaps get") echo '{"ok":true,"data":{"id":"swap_1","status":"completed"},"meta":{"command":"swaps get","duration":"1ms"}}'; exit 0 ;;
  "plans calculate_tax") case "$*" in *DE*) echo '{"ok":true,"data":{"currency":"usd","tax_behavior":"exclusive","subtotal":1000,"tax_amount":190,"total":1190,"status":"calculated"},"meta":{"command":"plans calculate_tax","duration":"1ms"}}' ;; *) echo '{"ok":true,"data":{"currency":"usd","tax_behavior":"exclusive","subtotal":1000,"tax_amount":0,"total":1000,"status":"calculated"},"meta":{"command":"plans calculate_tax","duration":"1ms"}}' ;; esac; exit 0 ;;
